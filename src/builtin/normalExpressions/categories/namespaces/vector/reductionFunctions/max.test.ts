...
describe('vec:max', () => {
  ...
  it('should calculate the moving max of a vector', () => {
    ...
    expect(() => lits.run('vec:moving-max([], 1)')).toThrowError(LitsError)
  })
  it('should calculate the centered moving max of a vector with padding', () => {
    ...
    expect(lits.run('vec:centered-moving-max([1], 1)')).toEqual([1])
    expect(() => lits.run('vec:centered-moving-max([], 0)')).toThrowError(LitsError)
  })
  it('should calculate the running max of a vector', () => {
    ...
    expect(() => lits.run('vec:running-max([], 1)')).toThrowError(LitsError)
  })
})
