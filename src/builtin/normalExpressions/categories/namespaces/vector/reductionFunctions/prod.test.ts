...
describe('vec:prod', () => {
  ...
  it('should calculate the moving product of a vector', () => {
    ...
    expect(lits.run('vec:moving-prod([1], 100)')).toEqual([1])
    expect(lits.run('vec:moving-prod([], 1)')).toEqual([])
  })
  it('should calculate the centered moving product of a vector with padding', () => {
    ...
    expect(lits.run('vec:centered-moving-prod([1], 100)')).toEqual([1])
    expect(lits.run('vec:centered-moving-prod([], 1)')).toEqual([])
  })
