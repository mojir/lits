...
describe('vec:median', () => {
  ...
  it('should calculate the centered moving median of a vector with padding', () => {
    ...
    expect(lits.run('vec:centered-moving-median([1, 2, 3, 4, 5, 6], 2)')).toEqual([null, 1.5, 2.5, 3.5, 4.5, 5.5])
    expect(lits.run('vec:centered-moving-median([1, 2, 3, 4, 5, 6], 2, 10)')).toEqual([5.5, 1.5, 2.5, 3.5, 4.5, 5.5])
    expect(lits.run('vec:centered-moving-median([1, 2, 3, 4, 5, 6], 3)')).toEqual([null, 2, 3, 4, 5, null])
  })
