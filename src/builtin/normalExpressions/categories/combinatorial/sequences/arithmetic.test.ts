...
describe('arithmetic', () => {
  ...
  it('should determine if numbers are in the sequence', () => {
    ...
    expect(lits.run('c:arithmetic?(1, 0, 1)')).toEqual(true)
    expect(lits.run('c:arithmetic?(1, 0, 2)')).toEqual(false)
    expect(lits.run('c:arithmetic?(1, 1, 0)')).toEqual(false)
    expect(lits.run('c:arithmetic?(1, 0.000000001, 1.1)')).toEqual(true)
    expect(lits.run('c:arithmetic?(1e-11, 0.01, 1e-11)')).toEqual(true)
  })
})
