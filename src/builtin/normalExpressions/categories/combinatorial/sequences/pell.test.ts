...
describe('pell', () => {
  ...
  it('should determine if numbers are in the sequence', () => {
    ...
    expect(lits.run('c:pell?(4)')).toEqual(false)
    expect(lits.run('c:pell?(5)')).toEqual(true)
    expect(lits.run('c:pell?(6)')).toEqual(false)
    expect(lits.run('c:pell?(7)')).toEqual(false)
    expect(lits.run('c:pell?(8)')).toEqual(false)
    expect(lits.run('c:pell?(9)')).toEqual(false)
  })
})
