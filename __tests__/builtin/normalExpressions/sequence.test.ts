  ...
  describe('nth', () => {
    it('array samples', () => {
      for (const lits2 of litsInstances) {
        expect(lits2.run('TEMP-nth([1, 2, 3], 1)')).toBe(2)
        expect(lits2.run('TEMP-nth([], 0)')).toBeNull()
        expect(lits2.run('TEMP-nth([1, 2, 3], 3)')).toBeNull()
        expect(lits2.run('TEMP-nth([1, 2, 3], -1)')).toBeNull()
        expect(lits2.run('TEMP-nth([1, 2, 3], -4)')).toBeNull()
        expect(() => lits2.run('TEMP-nth()')).toThrow(LitsError)
        expect(() => lits2.run('TEMP-nth({}, 1)')).toThrow(LitsError)
        expect(() => lits2.run('TEMP-nth([1, 2, 3])')).toThrow(LitsError)
        expect(() => lits2.run('TEMP-nth([1, 2, 3], 1, 2, 3)')).toThrow(LitsError)
      }
    })

    it('string samples', () => {
      expect(lits.run('TEMP-nth("A string", 1)')).toBe(' ')
      expect(lits.run('TEMP-nth("A string", 3)')).toBe('t')
      expect(lits.run('TEMP-nth("A string", -3)')).toBeNull()
      expect(lits.run('TEMP-nth("A string", 30)')).toBeNull()
      expect(lits.run('TEMP-nth("A string", -30)')).toBeNull()
      expect(() => lits.run('TEMP-nth("A string")')).toThrow(LitsError)
      expect(() => lits.run('TEMP-nth("A string", 1, 2, 3)')).toThrow(LitsError)
    })

    it('default values', () => {
      expect(lits.run('TEMP-nth([1, 2, 3], 1, 99)')).toBe(2)
      expect(lits.run('TEMP-nth([1, 2, 3], 3, 99)')).toBe(99)
      expect(lits.run('TEMP-nth([1, 2, 3], -1, 99)')).toBe(99)
      expect(lits.run('TEMP-nth([1, 2, 3], -4, 99)')).toBe(99)
      expect(lits.run('TEMP-nth("A string", 1, 99)')).toBe(' ')
      expect(lits.run('TEMP-nth("A string", 3, 99)')).toBe('t')
      expect(lits.run('TEMP-nth("A string", -3, 99)')).toBe(99)
      expect(lits.run('TEMP-nth("A string", 30, 99)')).toBe(99)
      expect(lits.run('TEMP-nth("A string", -30, 99)')).toBe(99)
    })

    it('null sequence', () => {
      expect(lits.run('TEMP-nth(null, 0)')).toBeNull()
      expect(lits.run('TEMP-nth(null, 0, 99)')).toBe(99)
    })
  })
