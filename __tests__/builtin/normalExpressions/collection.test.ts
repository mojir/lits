...
describe(`collection functions`, () => {
  ...
  describe(`has-some?`, () => {
    test(`samples`, () => {
      expect(lispish.run(`(has-some? [] [1])`)).toBe(false)
      expect(lispish.run(`(has-some? [1] [])`)).toBe(false)
      expect(lispish.run(`(has-some? [1] [1])`)).toBe(true)
      expect(lispish.run(`(has-some? [1 2 3] [0])`)).toBe(false)
      expect(lispish.run(`(has-some? [1 2 3] [0 1])`)).toBe(true)
      expect(lispish.run(`(has-some? (object) [:a])`)).toBe(false)
      expect(lispish.run(`(has-some? (object :a 1 :b 2) [0])`)).toBe(false)
      expect(lispish.run(`(has-some? (object :a 1 :b 2) [0 1])`)).toBe(true)
      expect(lispish.run(`(has-some? "Albert" "xyz")`)).toBe(false)
      expect(lispish.run(`(has-some? "Albert" ["Alb" "ert"])`)).toBe(false)
      expect(lispish.run(`(has-some? "Albert" ["A"])`)).toBe(true)
      expect(lispish.run(`(has-some? "Albert" "xyzl")`)).toBe(true)
      expect(lispish.run(`(has-some? [:a :b :c :d] "xyz")`)).toBe(false)
      expect(lispish.run(`(has-some? [:a :b :c :d] "xyzc")`)).toBe(true)

      expect(() => lispish.run(`(has-some? [] [1] 1)`)).toThrow()
      expect(() => lispish.run(`(has-some? [] 4)`)).toThrow()
      expect(() => lispish.run(`(has-some? [] true)`)).toThrow()
      expect(() => lispish.run(`(has-some? [] false)`)).toThrow()
      expect(() => lispish.run(`(has-some? [] nil)`)).toThrow()
      expect(() => lispish.run(`(has-some? [] odd?)`)).toThrow()
      expect(() => lispish.run(`(has-some? [] {})`)).toThrow()
      expect(() => lispish.run(`(has-some? true [1])`)).toThrow()
      expect(() => lispish.run(`(has-some? false [1])`)).toThrow()
      expect(() => lispish.run(`(has-some? nil [1])`)).toThrow()
      expect(() => lispish.run(`(has-some? odd? [1])`)).toThrow()
      expect(() => lispish.run(`(has-some? 3 [1])`)).toThrow()
    })
  })
