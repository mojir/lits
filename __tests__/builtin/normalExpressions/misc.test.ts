...
describe(`misc functions`, () => {
  ...
  describe(`get-path`, () => {
    test(`samples`, () => {
      expect(lits.run(`(get-path [1 2 3] "[1]")`)).toBe(2)
      expect(lits.run(`(get-path (object :a 1) :a)`)).toBe(1)
      expect(lits.run(`(get-path (object :a (object :b [1 2 3])) "a.b[1]")`)).toBe(2)
      expect(lits.run(`(get-path O "a.b[1]")`, { globals: { O: { a: { b: [1, 2, 3] } } } })).toBe(2)
      expect(lits.run(`(get-path O "a.c[1]")`, { globals: { O: { a: { b: [1, 2, 3] } } } })).toBe(null)
      expect(lits.run(`(get-path O "")`, { globals: { O: { a: { b: [1, 2, 3] } } } })).toEqual({
        a: { b: [1, 2, 3] },
      })
      expect(() => lits.run(`(get-path O)`, { globals: { O: { a: { b: [1, 2, 3] } } } })).toThrow()
      expect(() => lits.run(`(get-path)`, { globals: { O: { a: { b: [1, 2, 3] } } } })).toThrow()
      expect(() => lits.run(`(get-path O :a :b)`, { globals: { O: { a: { b: [1, 2, 3] } } } })).toThrow()
      expect(() => lits.run(`(get-path (regexp "abc" :a)`)).toThrow()
    })
  })
