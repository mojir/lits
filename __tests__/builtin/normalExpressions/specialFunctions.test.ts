  ...
  for (const lits of [new Lits(), new Lits({ debug: true })]) {
    describe(`string as function`, () => {
      test(`samples`, () => {
        ...
        expect(() => lits.run(`(:number ::number)`)).toThrow()
        expect(() => lits.run(`(::number {"firstName" "Albert", "lastName", "Mojir"})`)).toThrow()
        expect(() => lits.run(`({"firstName" "Albert", "lastName", "Mojir"} ::number)`)).toThrow()
