...
describe(`predicates`, () => {
  describe(`function?`, () => {
    test(`samples`, () => {
      ...
      expect(lispish.run(`(function? nil)`)).toBe(false)
  ...
  describe(`string?`, () => {
    test(`samples`, () => {
      ...
      expect(lispish.run(`(string? nil)`)).toBe(false)
  ...
  describe(`number?`, () => {
    test(`samples`, () => {
      ...
      expect(lispish.run(`(number? nil)`)).toBe(false)
  ...
  describe(`integer?`, () => {
    test(`samples`, () => {
      ...
      expect(lispish.run(`(integer? nil)`)).toBe(false)
  ...
  describe(`nil?`, () => {
    test(`samples`, () => {
      expect(lispish.run(`(nil? 1)`)).toBe(false)
      expect(lispish.run(`(nil? 0)`)).toBe(false)
      expect(lispish.run(`(nil? -1)`)).toBe(false)
      expect(lispish.run(`(nil? -1.123)`)).toBe(false)
      expect(lispish.run(`(nil? 0.123)`)).toBe(false)
      expect(lispish.run(`(nil? "")`)).toBe(false)
      expect(lispish.run(`(nil? :x)`)).toBe(false)
      expect(lispish.run(`(nil? [])`)).toBe(false)
      expect(lispish.run(`(nil? (object))`)).toBe(false)
      expect(lispish.run(`(nil? nil)`)).toBe(true)
      expect(lispish.run(`(nil? false)`)).toBe(false)
      expect(lispish.run(`(nil? true)`)).toBe(false)
      expect(lispish.run(`(nil? +)`)).toBe(false)
      expect(lispish.run(`(nil? +)`)).toBe(false)
      expect(() => lispish.run(`(nil?)`)).toThrow()
      expect(() => lispish.run(`(nil? true false)`)).toThrow()
    })
  })
  ...
  describe(`zero?`, () => {
    test(`samples`, () => {
      ...
      expect(() => lispish.run(`(zero? nil)`)).toThrow()
  ...
  describe(`pos?`, () => {
    test(`samples`, () => {
      ...
      expect(() => lispish.run(`(pos? nil)`)).toThrow()
  ...
  describe(`neg?`, () => {
    test(`samples`, () => {
      ...
      expect(() => lispish.run(`(neg? nil)`)).toThrow()
  ...
  describe(`even?`, () => {
    test(`samples`, () => {
      ...
      expect(() => lispish.run(`(even? nil)`)).toThrow()
  ...
  describe(`odd?`, () => {
    test(`samples`, () => {
      ...
      expect(() => lispish.run(`(odd? nil)`)).toThrow()
  ...
  describe(`array?`, () => {
    test(`samples`, () => {
      ...
      expect(lispish.run(`(array? nil)`)).toBe(false)
  ...
  describe(`coll?`, () => {
    test(`samples`, () => {
      expect(lispish.run(`(coll? 1)`)).toBe(false)
      expect(lispish.run(`(coll? 0)`)).toBe(false)
      expect(lispish.run(`(coll? -1)`)).toBe(false)
      expect(lispish.run(`(coll? -1.123)`)).toBe(false)
      expect(lispish.run(`(coll? 0.123)`)).toBe(false)
      expect(lispish.run(`(coll? "")`)).toBe(true)
      expect(lispish.run(`(coll? :x)`)).toBe(true)
      expect(lispish.run(`(coll? [])`)).toBe(true)
      expect(lispish.run(`(coll? [1 2 3])`)).toBe(true)
      expect(lispish.run(`(coll? (object))`)).toBe(true)
      expect(lispish.run(`(coll? {:a 1})`)).toBe(true)
      expect(lispish.run(`(coll? nil)`)).toBe(false)
      expect(lispish.run(`(coll? false)`)).toBe(false)
      expect(lispish.run(`(coll? true)`)).toBe(false)
      expect(lispish.run(`(coll? +)`)).toBe(false)
      expect(lispish.run(`(coll? +)`)).toBe(false)
      expect(() => lispish.run(`(coll?)`)).toThrow()
      expect(() => lispish.run(`(coll? true false)`)).toThrow()
    })
  })

  describe(`seq?`, () => {
    test(`samples`, () => {
      expect(lispish.run(`(seq? 1)`)).toBe(false)
      expect(lispish.run(`(seq? 0)`)).toBe(false)
      expect(lispish.run(`(seq? -1)`)).toBe(false)
      expect(lispish.run(`(seq? -1.123)`)).toBe(false)
      expect(lispish.run(`(seq? 0.123)`)).toBe(false)
      expect(lispish.run(`(seq? "")`)).toBe(true)
      expect(lispish.run(`(seq? :x)`)).toBe(true)
      expect(lispish.run(`(seq? [])`)).toBe(true)
      expect(lispish.run(`(seq? [1 2 3])`)).toBe(true)
      expect(lispish.run(`(seq? (object))`)).toBe(false)
      expect(lispish.run(`(seq? {:a 1})`)).toBe(false)
      expect(lispish.run(`(seq? nil)`)).toBe(false)
      expect(lispish.run(`(seq? false)`)).toBe(false)
      expect(lispish.run(`(seq? true)`)).toBe(false)
      expect(lispish.run(`(seq? +)`)).toBe(false)
      expect(lispish.run(`(seq? +)`)).toBe(false)
      expect(() => lispish.run(`(seq?)`)).toThrow()
      expect(() => lispish.run(`(seq? true false)`)).toThrow()
    })
  })
  ...
  describe(`object?`, () => {
    test(`samples`, () => {
      ...
      expect(lispish.run(`(object? nil)`)).toBe(false)
  ...
  describe(`regexp?`, () => {
    test(`samples`, () => {
      ...
      expect(lispish.run(`(regexp? nil)`)).toBe(false)
