...
describe('predicates', () => {
  for (const lits of [new Lits(), new Lits({ debug: true })]) {
    ...
    describe('collection?', () => {
      it('samples', () => {
        expect(lits.run('collection?(1)')).toBe(false)
        expect(lits.run('collection?(0)')).toBe(false)
        expect(lits.run('collection?(-1)')).toBe(false)
        expect(lits.run('collection?(-1.123)')).toBe(false)
        expect(lits.run('collection?(0.123)')).toBe(false)
        expect(lits.run('collection?("")')).toBe(true)
        expect(lits.run('collection?("x")')).toBe(true)
        expect(lits.run('collection?([])')).toBe(true)
        expect(lits.run('collection?([1, 2, 3])')).toBe(true)
        expect(lits.run('collection?({})')).toBe(true)
        expect(lits.run('collection?({ a: 1 })')).toBe(true)
        expect(lits.run('collection?(null)')).toBe(false)
        expect(lits.run('collection?(false)')).toBe(false)
        expect(lits.run('collection?(true)')).toBe(false)
        expect(lits.run('collection?(+)')).toBe(false)
        expect(lits.run('collection?(+)')).toBe(false)
        expect(() => lits.run('collection?()')).toThrow(LitsError)
        expect(() => lits.run('collection?(true, false)')).toThrow(LitsError)
      })
    })

    describe('sequence?', () => {
      it('samples', () => {
        expect(lits.run('sequence?(1)')).toBe(false)
        expect(lits.run('sequence?(0)')).toBe(false)
        expect(lits.run('sequence?(-1)')).toBe(false)
        expect(lits.run('sequence?(-1.123)')).toBe(false)
        expect(lits.run('sequence?(0.123)')).toBe(false)
        expect(lits.run('sequence?("")')).toBe(true)
        expect(lits.run('sequence?("x")')).toBe(true)
        expect(lits.run('sequence?([])')).toBe(true)
        expect(lits.run('sequence?([1, 2, 3])')).toBe(true)
        expect(lits.run('sequence?({})')).toBe(false)
        expect(lits.run('sequence?({ a: 1 })')).toBe(false)
        expect(lits.run('sequence?(null)')).toBe(false)
        expect(lits.run('sequence?(false)')).toBe(false)
        expect(lits.run('sequence?(true)')).toBe(false)
        expect(lits.run('sequence?(+)')).toBe(false)
        expect(lits.run('sequence?(+)')).toBe(false)
        expect(() => lits.run('sequence?()')).toThrow(LitsError)
        expect(() => lits.run('sequence?(true, false)')).toThrow(LitsError)
      })
    })
