...
describe('math functions', () => {
  describe('1+', () => {
    test('samples', () => {
      ...
      expect(() => lispish.run(`(1+ 1 1)`)).toThrow()
      expect(() => lispish.run(`(1+ "1")`)).toThrow()
      expect(() => lispish.run(`(1+ false)`)).toThrow()
      expect(() => lispish.run(`(1+ true)`)).toThrow()
      expect(() => lispish.run(`(1+ null)`)).toThrow()
      expect(() => lispish.run(`(1+ boolean)`)).toThrow()
      expect(() => lispish.run(`(1+ [])`)).toThrow()
      expect(() => lispish.run(`(1+ (object))`)).toThrow()
    })
  })
  ...
  describe('random', () => {
    test('samples', () => {
      ...
      expect(lispish.run(`(random 0.1)`)).toBeLessThan(0.1)
      expect(lispish.run(`(random 0.1)`)).toBeGreaterThanOrEqual(0)
    })
  })
